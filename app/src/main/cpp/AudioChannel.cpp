...
int AudioChannel::getPCM() {
    ...
    while(isPlaying) {
        ...
        if(NULL != clock) {//音视频同步，参考时钟为：音频
            if (frame->best_effort_timestamp != AV_NOPTS_VALUE) {
                *clock = av_q2d(time_base) * frame->best_effort_timestamp;//当前播放时间，单位：秒
            } else {
                *clock = 0;
            }
        }
...
void AudioChannel::audio_player() {
    ...
    if (SL_RESULT_SUCCESS != result) {
        LOGD("CreateAudioPlayer fail");
        return;
    }
    //  3.4 初始化播放器：SLObjectItf bqPlayerObject
    result = (*bqPlayerObject)->Realize(bqPlayerObject, SL_BOOLEAN_FALSE);
    if (SL_RESULT_SUCCESS != result) {
        LOGD("AudioPlayer Realize fail");
        return;
    }
    //  3.5 获取播放器接口：SLPlayItf bqPlayerPlay
    result = (*bqPlayerObject)->GetInterface(bqPlayerObject, SL_IID_PLAY, &bqPlayerPlay);
    if (SL_RESULT_SUCCESS != result) {
        LOGD("AudioPlayer GetInterface fail");
        return;
    }
