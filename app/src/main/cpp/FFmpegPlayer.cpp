...
FFmpegPlayer::FFmpegPlayer(const char *data_source, AVCallback *pCallback, GlobalContexts *global_context) {
...
//            this->pCallback->onError(105, THREAD_SUB);
        ...
        if (codecParameters->codec_type == AVMEDIA_TYPE_AUDIO) { // 音频流
            ...
            videoChannel = new VideoChannel(stream_index, codecContext, this->global_context);
            videoChannel->setRenderCallback(renderCallback);
            videoChannel->setTimeBase(stream->time_base);
            videoChannel->setAvgFrameRate(stream->avg_frame_rate);
            videoChannel->setReferenceClock(&referenceClock);
        }
    }

    if (audioChannel || videoChannel) {
        LOGD("prepare success duration : %ld", formatContext->duration / 1000000);//获取音视频时长
